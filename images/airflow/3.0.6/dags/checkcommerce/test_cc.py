...
_merchant_number_cache: str | None = None
_report_url_cache: str | None = None
...
def get_merchant_number() -> str:
    """Lazily retrieve merchant number at runtime, not import time."""
    global _merchant_number_cache
    if _merchant_number_cache is None:
        creds = get_check_commerce_credentials()
        _merchant_number_cache = creds.get("merchant_number")
    return _merchant_number_cache

def get_report_url() -> str:
    """Lazily retrieve report URL at runtime, not import time."""
    global _report_url_cache
    if _report_url_cache is None:
        creds = get_check_commerce_credentials()
        _report_url_cache = creds.get("report_url")
    return _report_url_cache
...
def checkcommerce_api_extract():
    # Lazily initialize values at runtime to avoid module-level Variable.get() calls
